...
export async function GET() {
  try {
    ...
    if (error) {
      throw new Error(`Failed to fetch planned income: ${error.message}`)
    }
    return successResponse(data || [])
  } catch (e) {
    return errorResponse(e, 'Failed to load planned income items')
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (error) {
      throw new Error(`Failed to create planned income: ${error.message}`)
    }
    return successResponse(data, 201)
  } catch (e) {
    return errorResponse(e, 'Failed to create planned income item')
  }
}
