...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (error) {
      throw new Error(`Failed to update planned income: ${error.message}`)
    }
    if (!data) {
      throw new ApiError('Planned income item not found', 404, 'NOT_FOUND')
    }
    return successResponse(data)
  } catch (e) {
    return errorResponse(e, 'Failed to update planned income item')
  }
}
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (error) {
      throw new Error(`Failed to delete planned income: ${error.message}`)
    }
    return successResponse({ deleted: true })
  } catch (e) {
    return errorResponse(e, 'Failed to delete planned income item')
  }
}
